...
// YandexClient talks to the Yandex Cloud Foundation Models completion API.
...
func (c *YandexClient) Complete(ctx context.Context, req CompleteRequest) (CompleteResponse, error) {
	...
	for _, m := range req.Messages {
		...
		msgs = append(msgs, yandexMessage{Role: role, Text: m.Content})
	}
	...
	// Yandex supports response schema constraining via jsonSchema (json_object/json_schema
	// generation config depending on API version); we pass it through when provided and
	// otherwise rely on prompt-level "respond with strict JSON" instructions.
	if req.JSONSchema != nil {
		body.JSONSchema = req.JSONSchema
	}
