...
Твоя задача — извлечь структурированный ProblemSpec СТРОГО в формате JSON, без пояснений вне JSON:
{
  ...
  "available_equipment": ["упомянутое оборудование цепи ЦЕЛИКОМ, вместе с любыми числовыми параметрами из текста (диаметр насадки, размер ячейки сита, модель, число секций и т.п.) — напр. если в тексте 'гидроциклоны (насадки 12 мм)', пиши ИМЕННО 'гидроциклоны (насадки 12 мм)', а не просто 'гидроциклоны'. Не отбрасывай цифры при обобщении до типа оборудования — они нужны для генерации конкретных, а не общих гипотез."],
